...
if (MODE_SAVE == $data['modus']) {
    if ($_POST['galerieeintrag_id']) {
        $Galerieeintrag->load($_POST['galerieeintrag_id']);
    }
    ...
    $Galerieeintrag->setGooglePhotosLink($_POST['google_photos_link']);
...
if (VIEW_DETAIL == $data['view']) {
    ...
    foreach ($AthletIDArray as $AthletID) {
        $data['mitglied_array'][] = new CMitglied((int) $AthletID);
    }
...
if (VIEW_LIST == $data['view']) {
    ...
    while ($row = mysqli_fetch_row($result)) {
        $data['galerie_array'][] = new CGalerieeintrag($row[0]);
    }
}
