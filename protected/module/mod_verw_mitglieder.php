...
if (CDBConnection::getInstance()->getAthLock()) {
    throw new CShowError(STR_ATH_LOCK);
}
...
if (MODE_SAVE == $data['modus']) {
    ...
    $Mitglied->setBeitrag($_POST['beitrag']);
...
if (VIEW_LIST == $data['view']) {
    ...
    if ($data['fltr2']) {
        $query_where[] = 'makl.aklagruppe'.((4 == $data['fltr2']) ? (' IS NULL') : ('='.$data['fltr2']));
    }

    foreach ($query_where as $i => $clause) {
        $query .= (($i) ? (' AND ') : (' WHERE ')).$clause;
    }
