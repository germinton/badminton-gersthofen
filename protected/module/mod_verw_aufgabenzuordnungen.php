...
if(VIEW_DETAIL == $data['view'])
{
	...
	while($row = mysql_fetch_row($result)) {$data['mitglieder_array'][] = new CMitglied($row[0]);}
...
if(VIEW_LIST == $data['view'])
{
	...
	while($row = mysql_fetch_row($result)) {$data['fltr1_array'][] = new CMitglied($row[0]);}
	...
	while($row = mysql_fetch_row($result)) {$data['fltr2_array'][] = new CAufgabe($row[0]);}
	...
	if(!$result = mysql_query($query)) {throw new Exception(mysql_error(CDBConnection::getDB()));}
	while($row = mysql_fetch_row($result)) {$data['aufgabenzuordnungen_array'][] = new CAufgabenzuordnung($row[0]);}
}
