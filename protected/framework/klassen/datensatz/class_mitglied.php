...
class CMitglied extends CAthlet
{
    ...
    public function __toString()
    {
        ...
        if (is_string($this->mSpitzname)) {
            return $this->mSpitzname;
        }
        if (is_string(CAthlet::getVorname())) {
            return CAthlet::getVorname();
        }
    ...
    {
        $this->mBeitrag = (($s = htmlspecialchars(trim((string) $Beitrag))) ? ($s) : (null));
    }
    ...
    {
        $FlagArray = (array) $FlagVariant;
        if (is_null($v = $this->mEMail) and in_array(GET_NBSP, $FlagArray)) {
            return '&nbsp;';
        }
    ...
    public function load($AthletID)
    {
        ...
        $format = 'SELECT geburtstag, strasse, plz, ort, tel_priv, tel_priv2, tel_gesch, fax, tel_mobil, email, website, '.
                         ...
                         'erzber_tel_mobil, erzber_email, beitrag_id '.
    ...
    public function store()
    {
        ...
        if (self::isValidID($this->getID())) {
            $format = 'UPDATE athleten_mitglieder SET '.
                      ...
                      'erzber_tel_mobil=%s, erzber_email=%s, beitrag_id=%s '.
                      'WHERE athlet_id=%s';
            ...
            sS($this->mErzBerEMail), sD($this->mBeitrag), $this->getID());
        } else {
            ...
            sS($this->mErzBerVorname), sS($this->mErzBerNachname), sS($this->mErzBerTelMobil), sS($this->mErzBerEMail), sD($this->mBeitrag));
        }
        if (!$result = mysqli_query(CDriveEntity::getDB(), $query)) {
            throw new Exception(mysqli_error(CDriveEntity::getDB()));
        }
    ...
    public function check()
    {
        ...
        if (!is_null($this->mEMail)) {
            ...
            } elseif (!preg_match(REGEX_EMAIL, $this->mEMail)) {
        ...
        if (!is_null($this->mUeberSich)) {
            ...
            } elseif (substr_count($this->mUeberSich, '{') != substr_count($this->mUeberSich, '}')) {
        ...
        } elseif (!preg_match(REGEX_STDCHR, $this->mBenutzername)) {
        ...
        if (!is_null($this->mErzBerEMail)) {
            ...
            } elseif (!preg_match(REGEX_EMAIL, $this->mErzBerEMail)) {
    ...
    public function updateLastLogin()
    {
        if (self::isValidID($this->getID())) {
            ...
            throw new Exeption('updateLastLogin() nicht möglich da athlet_id ungültig!');
        }
        if (!$result = mysqli_query(CDriveEntity::getDB(), $query)) {
            throw new Exception(mysqli_error(CDriveEntity::getDB()));
        }
    ...
    public function getAlter($FlagVariant = array(), $Stichtag = null)
    {
        ...
        $row = mysqli_fetch_row($result);
        if (!$row) {
            throw new Exception('Mitglied mit athlet_id='.$this->getAthletID().' nicht gefunden!');
        }
    ...
    public function getAltersklasse($FlagVariant = array(), $Stichtag = null)
    {
        ...
        if ($AKlasse and in_array(GET_C2SC, $FlagArray)) {
            $AKlasse = C2S_Altersklasse($AKlasse);
        }
    ...
    public function getAKlaGruppe($FlagVariant = array(), $Stichtag = null)
    {
        ...
        if ($AKlaGruppe and in_array(GET_C2SC, $FlagArray)) {
            $AKlaGruppe = C2S_AKlaGruppe($AKlaGruppe);
        }
        ...
        foreach ($AufgabeIDArray as $AufgabeID) {
            switch ($AufgabeID) {
                case S_STAFFELLEITER:
                    ...
                    if (S_HERR == CAthlet::getAnrede()) {
                        $Aufgabenstring = $Aufgabe->getBezMaennlich();
                    } elseif (S_DAME == CAthlet::getAnrede()) {
                        $Aufgabenstring = $Aufgabe->getBezWeiblich();
                    }
                    break;
                case S_MANNSCHAFTSFUEHRER:
                    ...
                    if (S_HERR == CAthlet::getAnrede()) {
                        $Aufgabenstring = $Aufgabe->getBezMaennlich().' ('.$Mannschaft.')';
                    } elseif (S_DAME == CAthlet::getAnrede()) {
                        $Aufgabenstring = $Aufgabe->getBezWeiblich().' ('.$Mannschaft.')';
                    }
                    break;
                default:
        ...
        foreach ($AufgabeIDArray as $AufgabeID) {
            switch ($AufgabeID) {
                case S_STAFFELLEITER:
                    $Aufgabe->load(S_STAFFELLEITER);
                    if (S_HERR == CAthlet::getAnrede()) {
                        $Aufgabenstring = '<span style="font-weight: bold">'.$Aufgabe->getBezMaennlich().'</span>';
                    } elseif (S_DAME == CAthlet::getAnrede()) {
                        $Aufgabenstring = '<span style="font-weight: bold">'.$Aufgabe->getBezWeiblich().'</span>';
                    }
                    break;
                case S_MANNSCHAFTSFUEHRER:
                    $Aufgabe->load(S_MANNSCHAFTSFUEHRER);
                    ...
                    $Mannschaft->load($row[0]);
                    if (S_HERR == CAthlet::getAnrede()) {
                        $Aufgabenstring = '<span style="font-weight: bold">'.$Aufgabe->getBezMaennlich().'</span><br />('.$Mannschaft.')';
                    } elseif (S_DAME == CAthlet::getAnrede()) {
                        $Aufgabenstring = '<span style="font-weight: bold">'.$Aufgabe->getBezWeiblich().'</span><br />('.$Mannschaft.')';
                    }
                    break;
                default:
                    $query = 'SELECT aufgabenzuordnung_id FROM aufgabenzuordnungen '.
    ...
    public function statEinsaetzePlatz($String = 'AJS', $BeachteGeschlecht = false)
    {
        ...
        if ($BeachteGeschlecht) {
            $query .= ' INNER JOIN athleten a ON me.athlet_id=a.athlet_id ';
        }
        $query_where = $this->statEinsaetzeQueryWhereString($String);
        ...
        if ($BeachteGeschlecht) {
            $query .= (strlen($query_where) ? (' AND ') : (' WHERE ')).' a.anrede='.CAthlet::getAnrede();
        }
    ...
    private function statEinsaetzeQueryWhereString($String)
    {
        ...
        if (strpos($String, 'A')) {
            $query_where_or_parts[] = 'aklagruppe='.S_AKTIVE;
        }
        if (strpos($String, 'J')) {
            $query_where_or_parts[] = 'aklagruppe='.S_JUGEND;
        }
        if (strpos($String, 'S')) {
            $query_where_or_parts[] = 'aklagruppe='.S_SCHUELER;
        }
        foreach ($query_where_or_parts as $i => $clause) {
            $query_where[0] .= ($i ? ' OR ' : '').$clause;
        }
        if (strlen($query_where[0])) {
            $query_where[0] = '('.$query_where[0].')';
        }
        ...
        $query_where_or_parts = array();
        if (strpos($String, 'E')) {
            $query_where_or_parts[] = 'spielart='.S_EINZEL;
        }
        if (strpos($String, 'D')) {
            $query_where_or_parts[] = 'spielart='.S_DOPPEL;
        }
        if (strpos($String, 'M')) {
            $query_where_or_parts[] = 'spielart='.S_MIXED;
        }
        foreach ($query_where_or_parts as $i => $clause) {
            $query_where[1] .= ($i ? ' OR ' : '').$clause;
        }
        if (strlen($query_where[1])) {
            $query_where[1] = '('.$query_where[1].')';
        }
        ...
        for ($i = 1; $i < 10; ++$i) {
            if (strpos($String, (string) $i)) {
                $query_where_or_parts[] = 'nr='.$i;
            }
        }
        foreach ($query_where_or_parts as $i => $clause) {
            $query_where[2] .= (($i) ? (' OR ') : ('')).$clause;
        }
        if (strlen($query_where[2])) {
            $query_where[2] = '('.$query_where[2].')';
        }

        $returnstring = '';
        foreach ($query_where as $clause) {
            if ($clause) {
                $returnstring .= ((strlen($returnstring)) ? (' AND ') : ('')).$clause;
            }
        }
    ...
    public function getBeitragArray()
    {
        $beitraege = array();
        $query = 'SELECT beitrag_id FROM beitraege';
        if (!$result = mysqli_query(CDriveEntity::getDB(), $query)) {
            throw new Exception(mysqli_error(CDriveEntity::getDB()));
        }
        while ($row = mysqli_fetch_row($result)) {
            $TmpBeitrag = new CBeitrag($row[0]);
            $beitraege[] = $TmpBeitrag;
        }
